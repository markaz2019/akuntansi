<div class=" kotak animated slideInDown" style="z-index:1">
	...
	<?php } else if (!empty($tanggal) && empty($result)) { ?>
		...
		<form class="form-inline ml-auto " style="float:right">
			<div class="form-group ">
				<input type="text" class="form-search" placeholder="Search" id="myInput">
			</div>
			<div class="form-group">
				<button class=" btn btn-neutral">Excel</button>
			</div>
		</form><br>
		<table id="tabel" class="table table-striped table-bordered kotak bounceInDown" width="100%" tabindex="0" aria-label="results" style="z-index:1">
			<thead>
				<tr>
					<th>No</th>
	...
	<?php } else if (!empty($tanggal) && !empty($result)) { ?>
		...
		<table id="tabel" class="table table-hover kotak bounceInDown" width="100%" tabindex="0" aria-label="results">
			<thead>
				<tr>
					<th>No</th>
					<th scope="col">Modul ID</th>
